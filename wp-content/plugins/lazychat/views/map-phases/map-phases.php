<?php
/**
 * This file contains HTML codes to map phases.
 *
									...
									<div class="card-body">
										<div class="row d-flex align-items-center">
											<div class="col-sm-12 col-xl-6 col-lg-6 d-flex flex-column">
												<label for="' . esc_html( $status['name'] ) . '" class="font-weight-bold font-sm" style="font-size: 15px;">' . esc_html( $status['name'] ) . '</label>
												<small class="text-muted" style="font-size: 12px;">' . esc_html( $status['summary'] ) . '</small>
											</div>
											<div class="col-sm-12 col-xl-6 col-lg-6 d-flex flex-column align-items-end">';

						if ( 'pending' === $status['code'] ) {
							echo '<label for="pending" class="d-flex justify-content-end font-weight-bold" style="font-size: 15px;">Pending</label>';
						} elseif ( 'completed' === $status['code'] ) {
							echo '<label for="delivered" class="d-flex justify-content-end font-weight-bold" style="font-size: 15px;">Delivered</label>';
						} else {
							echo '<select class="form-control select2 lazychat-order-phase pb-2" name="order_phase[' . esc_html( $status['code'] ) . ']" id="order-phase-' . esc_html( $key ) . '" data-code="' . esc_html( $status['code'] ) . '" required>
														<option value="">Select Order Phase</option>';
							foreach ( $_SESSION['lazychat_order_phases'] as $phase ) {
								echo '<option value="' . esc_html( $phase['id'] ) . '"' . ( $phase['woocommerce_code'] === $status['code'] ? 'selected' : '' ) . '>'
									. esc_html( $phase['name'] ) .
									'</option>';
							}
							echo '<option value="create-new">Create New</option>
									</select>
									<input class="form-control mt-2" id="create-new" name="create_new[' . esc_html( $status['code'] ) . ']" type="text" placeholder="Create new phase" style="display: none;" required />';
						}
						echo '</div>
									</div>
								</div>
